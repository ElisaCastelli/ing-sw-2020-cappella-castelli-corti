...
public class CLIView extends View {
    ...
    public void updateBoard(UpdateBoardEvent updateBoardEvent) {
        ...
        printBoard(updateBoardEvent.isShowReachable());
        if(!isPlaying){
    ...
    @Override
    public void initialize(ObjInitialize objInitialize) {
        setBoard(objInitialize.getBoard());
        setUsers(objInitialize.getUserArray());
        printBoard(false);
        if(!isPlaying){
            sendMessageToServer.sendAckState();
        }
    ...
    public void askPlayer(){
        new Thread(() -> {
            ...
            sendMessageToServer.sendPlayer(name, age);
        }).start();
    }
    ...
    public void setIndexPlayer(ObjState objState){
        ...
        if(indexPlayer == objState.getCurrentPlayer()) {
            isPlaying=true;
    ...
    @Override
    public void setPlaying(ObjState objState){
        if (indexPlayer == objState.getCurrentPlayer()) {
            isPlaying = true;
    ...
    public void ask3Card(ArrayList<String> cards) {
        ...
        if(isPlaying) {
            ...
            System.out.println("Someone choosing the "+ nPlayer +" cards");
            sendMessageToServer.sendAckState();
        }
    }
